...
function pranayama_yoga_scripts() {
  ...
  wp_enqueue_script( 'equal-height', get_template_directory_uri() . '/js' . $build . '/equal-height' . $suffix . '.js', array('jquery'), '0.7.0', PRANAYAMA_YOGA_THEME_VERSION, true );
...
function pranayama_yoga_change_comment_form_default_fields( $fields ){
    ...
    $fields['author'] = '<p class="comment-form-author"><label class="screen-reader-text" for="author">' . esc_html__( 'Name*', 'pranayama-yoga' ) . '</label><input id="author" name="author" placeholder="' . esc_attr__( 'Name*', 'pranayama-yoga' ) . '" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' /></p>';
    
    $fields['email'] = '<p class="comment-form-email"><label class="screen-reader-text" for="email">' . esc_html__( 'Email*', 'pranayama-yoga' ) . '</label><input id="email" name="email" placeholder="' . esc_attr__( 'Email*', 'pranayama-yoga' ) . '" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' /></p>';
...
function pranayama_yoga_change_comment_form_defaults( $defaults ){
    ...
    $defaults['comment_field'] = '<p class="comment-form-comment"><label class="screen-reader-text" for="comment">' . esc_html__( 'Comment', 'pranayama-yoga' ) . '</label><textarea id="comment" name="comment" placeholder="' . esc_attr__( 'Comment', 'pranayama-yoga' ) . '" cols="45" rows="8" aria-required="true"></textarea></p>';
        ...
        <div class="welcome-message notice notice-info">
            <div class="notice-wrapper">
                <div class="notice-text">
                    ...
                    <p><?php printf( __( '%1$s is now installed and ready to use. Click below to see theme documentation, plugins to install and other details to get started.', 'pranayama-yoga' ), $name ) ; ?></p>
...
if( ! function_exists( 'pranayama_yoga_restore_admin_notice' ) ) :
/**
 * Restoring admin notice on theme switch
 */
function pranayama_yoga_restore_admin_notice(){
    update_option( 'pranayama_yoga_admin_notice', false );
}
endif;
add_action( 'after_switch_theme', 'pranayama_yoga_restore_admin_notice' );
